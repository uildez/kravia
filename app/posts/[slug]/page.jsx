...
export async function generateMetadata({ params }) {
  ...
  return {
    title: post?.seoTitle || 'Título padrão',
    description: post?.seoDescription || 'Descrição padrão do blog',
    keywords: post?.seoKeywords?.join(', ') || 'blog, artigos',
    openGraph: {
      title: post?.seoTitle,
      description: post?.seoDescription,
      images: post?.imagem?.asset?.url
        ? [
          {
            url: post.imagem.asset.url,
            width: 1200,
            height: 630,
            alt: post.seoTitle || 'Imagem do post'
          }
        ]
        : []
    },
    twitter: {
      ...
      title: post?.seoTitle,
      description: post?.seoDescription,
      images: post?.imagem?.asset?.url ? [post.imagem.asset.url] : []
    }
  }
}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-x-20 w-full mt-12 px-8 lg:px-20 2xl:px-40 min-h-[60vh]">
        <div className="flex flex-col items-start col-span-1 lg:col-span-3 gap-4">
          <h1 className="text-3xl lg:text-5xl font-bold font-aeonik text-center lg:text-left">
            {post.title}
          </h1>
        ...
        <div className="flex flex-col gap-8 col-span-1 lg:col-span-2">
          ...
          <div className="flex flex-col gap-4">
            <h2 className="text-xl lg:text-3xl font-bold font-aeonik text-center lg:text-left">
              Últimos artigos
            </h2>
